            ...
            .then(({ rankings, last_updated_at }) => {
                ...
                rankings.forEach(({ nick, blocks, total_claims }, idx) => {
                    ...
                    li.append(
                        ...
                        b_bl,
                        ` block${blocks.length > 1 ? 's' : ''} controlled: `
                    );
            ...
            .then(({ blocks, last_updated_at }) => {
                ...
                for (let i = 0; i < 256; i++) {
                    ...
                    if (blockIdx >= 0) {
                        ...
                        const nickField = el.querySelector('.s-nick');

                        el.classList.remove('s-empty');
                        el.querySelector('.s-count').innerText = `(${
                            b.claims
                        } address${b.claims == 1 ? '' : 'es'})`;

                        if (b.is_tied) {
                            nickField.innerText = '(tied)';
                            el.classList.add('s-tied');
                        } else {
